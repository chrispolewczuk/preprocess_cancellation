...
def preprocess_simplify3d(infile):
    known_objects: Dict[str, KnownObject] = {}
    current_hull: Optional[HullTracker] = None
    for line in infile:
        if line.startswith("; process "):
            object_id = line.split("; process ")[1].strip()
        ...
        if line.startswith("; end exclusion"):
            current_hull = None
    ...
    for line in infile:
        yield line
        if line.startswith("; end exclusion") and not object_id is None:
            yield from object_end_marker(known_objects[object_id].name)        
        if line.startswith("; process "):
            if object_id is not None:
                yield from object_end_marker(known_objects[object_id].name)                
            object_id =line.split("; process ")[1].strip()
            yield from object_start_marker( known_objects[object_id].name)
...
SLICERS: dict[str, Tuple[str, callable]] = {
    ...
    "simplify3d": ("; G-Code generated by Simplify3D", preprocess_simplify3d),
}
